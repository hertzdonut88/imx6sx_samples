...
char *usage = "Usage: ./imx6sx_vdac_2d "\
	       "-H display this help \n "\
	       "\n"\
...
/* Simply memcpy the image to the framebuffer, not the most efficent but it works ! */
void copy_image_to_fb(int left, int top, int width, int height, uint *img_ptr, struct fb_var_screeninfo *screen_info)
{
	int i;
	...
	dbg(DBG_DEBUG," left %d top %d width %d height %d xres %d yres %d bpp %d \n", left,top,width,height,screen_info->xres,screen_info->yres,screen_info->bits_per_pixel);
...
int v4l_capture_setup(void) {
	...
	dbg(DBG_INFO,"\t WxH@fps = %dx%d@%d \t Image size = %d \n", fmt.fmt.pix.width,
			fmt.fmt.pix.height, parm.parm.capture.timeperframe.denominator,fmt.fmt.pix.sizeimage);
...
int memalloc(int buf_size, int buf_cnt) {
	...
	for (i = 0; i < buf_cnt; i++) {
		...
		dbg(DBG_DEBUG, "USRP: alloc bufs va=0x%x, pa=0x%x, size %d\n",
				(unsigned int) buffers[i].start, (unsigned int) buffers[i].offset, buf_size);
	}
...
int vadc_test(void *arg) {
	...
	do {
		...
		copy_image_to_fb(0, 0, width, height, (uint *)mem_o.virt_uaddr, &var);
...
int main(int argc, char *argv[]) {
	...
	pthread_create(&sigtid, NULL, (void *)&signal_thread, NULL);
   	...
   	pthread_create(&input_arg.tid, NULL,
	   		(void *)&vadc_test, (void *)&input_arg.cmd);
